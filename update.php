...
$image_err = $property_num_err = $new_property_num_err = $description_err = $quantity_unit_err = $unit_value_err = $BPC_quantity_err = $BPC_value_err = $OPC_quantity_err = $OPC_value_err = $SO_quantity_err = $SO_value_err = $Remarks_err = $type_err = $date_err = $issued_err = "";
 if(isset($_GET['id'])) {
...
if(isset($_POST["id"]) && !empty($_POST["id"])){
   ...
   // Check input errors before inserting in database
    if(empty($image_err) && empty($property_num_err) && empty($new_property_num_err) && empty($description_err) && empty($quantity_unit_err) && empty($unit_value_err) && empty($BPC_quantity_err) && empty($BPC_value_err) && empty($OPC_quantity_err) && empty($OPC_value_err) && empty($SO_quantity_err) && empty($SO_value_err) && empty($date_err) && empty($issued_err) && empty($Remarks_err) && empty($type_err)){
        // Prepare an insert statement
        $sql = "UPDATE stock SET article = :article, property_num = :property_num, new_property_num= :new_property_num, description = :description, quantity_unit = :quantity_unit, unit_value = :unit_value, BPC_quantity = :BPC_quantity, BPC_value = :BPC_value, OPC_quantity = :OPC_quantity, OPC_value = :OPC_value, SO_quantity = :SO_quantity, SO_value = :SO_value, date = :date, Remarks= :Remarks, image= :image, issued= :issued, type= :type  WHERE id= :id";
...
<head>
    <meta charset="UTF-8">
    <title>CHRIIS-Update Item</title>
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    ...
                    <center><h2 style="font-size: 50px;"><strong>Update Item</strong></h2></center>
                    </div>
                    <h3>Please edit the input values and don't forget to upload new image before submit it to update the record.</h3>
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data">
						<div class="form-group <?php echo (!empty($image_err)) ? 'has-error' : ''; ?>">
                            <label style="font-size: 20px">Image</label>
                            <input style="font-size: 20px" type="file" name="image" value="">
							<h4>Image Preview: <?php echo $image;?> </h4>
							<center><img width="300" height="300" src="uploads/<?php echo $image ?>" /></center>
                            <span class="help-block"><h3><?php echo $image_err;?></h3></span>
                        </div>
